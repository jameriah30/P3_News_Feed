...
class Survey
{
    ...
    public function __construct($id)
    {
        ...
        //SELECT * FROM `wn16_questions` WHERE Survey!D=1;


    # SQL statement - PREFIX is optional way to distinguish your app
    $sql = "select * FROM wn16_questions WHERE SurveyID=$this->SurveyID";
    ...
    if (mysqli_num_rows($result) > 0) {#there are records - present data
       // $this->IsValid = true;//data exists
        while ($row = mysqli_fetch_assoc($result)) {# pull data from associative array
            ...
            $this->Questions[] = new Question($row['QuestionId'],$row['Question']);
            //$this->Title = $row['Title'];
            //$this->Description = $row['Description'];
...
class Question
{
    ...
    public function __construct($QuestionId, $QuestionName)
    {
